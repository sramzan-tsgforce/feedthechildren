  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.__('Remove','functionality-for-blogpostlite-theme').'" title="'.__('Remove','functionality-for-blogpostlite-theme').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.__('Add','functionality-for-blogpostlite-theme').'" title="'.__('Add','functionality-for-blogpostlite-theme').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  if (!isset($value['id']))
	...
	echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
	if ($has_image)
		echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".__('Remove Image', 'functionality-for-blogpostlite-theme')."' type='button'/>";
	else
		echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".__('Upload Image', 'functionality-for-blogpostlite-theme')."' type='button'/>";
	$this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_color( $field, $meta ) {
    ...
    //  echo "<a href='#' class='at-color-select button' rel='{$field['id']}'>" . __( 'Select a color' ) . "</a>";
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color','functionality-for-blogpostlite-theme' ) . "'/>";
