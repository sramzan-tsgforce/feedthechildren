...
class RecentPostsPlus extends WP_Widget {
    ...
    private $default_config = array(
        ...
        <div class="news-item {THUMB}">
            <div class="post_container">
                ...
                <div class="post-info">
                    {CATEGORIES_C}
                    <span class="post-comments icon-comment-empty">{COMMENT_COUNT}</span>
                </div>
    ...
    function __construct() {
        $widget_ops = array(
            ...
            'description' => __('The most recent posts on your site. Includes advanced options.', 'functionality-for-blogpostlite-theme')
        );
        parent::__construct('recent-posts-plus', __('BlogPost - Recent Posts Plus', 'functionality-for-blogpostlite-theme'), $widget_ops);
    }
    ...
    function widget( $args, $instance ) {
        ...
        if(stripos(ltrim($widget_output_template), '<li') === 0)
            ...
            echo '<div class="recent-post-plus">'.$output.'</div>';
    ...
    function parse_output($instance) {
        ...
        if( $the_query->have_posts() ) {
            ...
            while ( $the_query->have_posts() ) { $the_query->the_post();
                ...
                if($include_post_thumbnail == "false" || get_the_post_thumbnail($ID, 'blogpost-gallery-medium') == '' ) {
                    $POST_THUMBNAIL = '';
                    $extra_class = "no_thumb";
                } else {
                    $extra_class = "thumb";
                    if ( $post_thumbnail_width ==  '' || $post_thumbnail_height == '' ) {
                        $POST_THUMBNAIL = get_the_post_thumbnail($ID, 'blogpost-gallery-medium');
                    } else {
                ...
                $categories_container = '<span class="post-categories ' . vh_get_random_circle() . '">' . $categories . '</span>';
    ...
    function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php _e('Title:', 'functionality-for-blogpostlite-theme'); ?></label>
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id('title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('title') ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
        </p>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id('count') ); ?>"><?php _e('Number of posts to show:', 'functionality-for-blogpostlite-theme'); ?></label>
            <input id="<?php echo esc_attr( $this->get_field_id('count') ); ?>" name="<?php echo esc_attr( $this->get_field_name('count') ); ?>" type="text" size="3" value="<?php echo esc_attr( $count ); ?>" />
        </p>

        <p>
            <?php if( current_theme_supports('post-thumbnails') ): ?>
                <input id="<?php echo esc_attr( $this->get_field_id('include_post_thumbnail') ); ?>" name="<?php echo esc_attr( $this->get_field_name('include_post_thumbnail') ); ?>" type="checkbox" value="true"  class="checkbox" <?php echo ($include_post_thumbnail == 'true') ? 'checked="checked"' : '' ?> />
                <label for="<?php echo esc_attr( $this->get_field_id('include_post_thumbnail') ); ?>"><?php _e('Include post thumbnail', 'functionality-for-blogpostlite-theme'); ?></label><br>
            <?php endif; ?>

            <input id="<?php echo esc_attr( $this->get_field_id('include_post_excerpt') ); ?>" name="<?php echo esc_attr( $this->get_field_name('include_post_excerpt') ); ?>" type="checkbox" value="true" class="checkbox" <?php echo ($include_post_excerpt == 'true') ? 'checked="checked"' : '' ?> />
            <label for="<?php echo esc_attr( $this->get_field_id('include_post_excerpt') ); ?>"><?php _e('Include post excerpt', 'functionality-for-blogpostlite-theme'); ?></label><br>

            <br>

            <input class="rpp_show-expert-options checkbox" id="<?php echo esc_attr( $this->get_field_name('show_expert_options') ); ?>" name="<?php echo esc_attr( $this->get_field_name('show_expert_options') ); ?>" type="checkbox" value="true" <?php echo ($show_expert_options == 'true') ? 'checked="checked"' : '' ?> />
            <label for="<?php echo esc_attr( $this->get_field_id('show_expert_options') ); ?>"><?php _e('Show expert options', 'functionality-for-blogpostlite-theme'); ?></label>
        </p>

        <div class="rpp_expert-panel" style="display:none; margin-top: 10px">

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id('truncate_post_title') ); ?>"><?php _e('Limit post title:', 'functionality-for-blogpostlite-theme'); ?></label>
                <input id="<?php echo esc_attr( $this->get_field_id('truncate_post_title') ); ?>" name="<?php echo esc_attr( $this->get_field_name('truncate_post_title') ); ?>" type="text" size="2" value="<?php echo esc_attr( $truncate_post_title ); ?>" />
                <select id="<?php echo esc_attr( $this->get_field_id('truncate_post_title_type') ); ?>" name="<?php echo esc_attr( $this->get_field_name('truncate_post_title_type') ); ?>">
                ...
                <label for="<?php echo esc_attr( $this->get_field_id('truncate_post_excerpt') ); ?>"><?php _e('Limit post excerpt:', 'functionality-for-blogpostlite-theme'); ?></label>
                <input id="<?php echo esc_attr( $this->get_field_id('truncate_post_excerpt') ); ?>" name="<?php echo esc_attr( $this->get_field_name('truncate_post_excerpt') ); ?>" type="text" size="2" value="<?php echo esc_attr( $truncate_post_excerpt ); ?>" />
                <select id="<?php echo esc_attr( $this->get_field_id('truncate_post_excerpt_type') ); ?>" name="<?php echo esc_attr( $this->get_field_name('truncate_post_excerpt_type') ); ?>">
                ...
                <label for="<?php echo esc_attr( $this->get_field_id('truncate_elipsis') ); ?>"><?php _e('Limit ellipsis:', 'functionality-for-blogpostlite-theme'); ?></label>
                <input id="<?php echo esc_attr( $this->get_field_id('truncate_elipsis') ); ?>" name="<?php echo esc_attr( $this->get_field_name('truncate_elipsis') ); ?>" type="text" size="3" value="<?php echo esc_attr( $truncate_elipsis ); ?>" /><br>

                <br>

                <label for="<?php echo esc_attr( $this->get_field_id('post_date_format') ); ?>"><?php _e('Post date format:', 'functionality-for-blogpostlite-theme'); ?></label> <a title="View Documentation" target="_blank" href="http://www.pjgalbraith.com/2011/08/recent-posts-plus/">(?)</a>
                <input id="<?php echo esc_attr( $this->get_field_id('post_date_format') ); ?>" name="<?php echo esc_attr( $this->get_field_name('post_date_format') ); ?>" type="text" size="3" value="<?php echo esc_attr( $post_date_format ); ?>" />
            </p>

            <?php if( current_theme_supports('post-thumbnails') ): ?>
                <p>
                    <label for="<?php echo esc_attr( $this->get_field_id('post_thumbnail_width') ); ?>"><?php _e('Thumbnail size (WxH):', 'functionality-for-blogpostlite-theme'); ?></label>
                    <input id="<?php echo esc_attr( $this->get_field_id('post_thumbnail_width') ); ?>" name="<?php echo esc_attr( $this->get_field_name('post_thumbnail_width') ); ?>" type="text" size="3" value="<?php echo esc_attr( $post_thumbnail_width ); ?>" style="width: 40px" />
                    <input id="<?php echo esc_attr( $this->get_field_id('post_thumbnail_height') ); ?>" name="<?php echo esc_attr( $this->get_field_name('post_thumbnail_height') ); ?>" type="text" size="3" value="<?php echo esc_attr( $post_thumbnail_height ); ?>" style="width: 40px" />
                </p>
            <?php endif; ?>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id('wp_query_options') ); ?>"><?php _e('WP_Query Options', 'functionality-for-blogpostlite-theme'); ?></label> <a title="View Documentation" target="_blank" href="http://www.pjgalbraith.com/2011/08/recent-posts-plus/">(?)</a><br />
                <textarea id="<?php echo esc_attr( $this->get_field_id('wp_query_options') ); ?>" name="<?php echo esc_attr( $this->get_field_name('wp_query_options') ); ?>" style="width:222px; height: 100px; font-size: 80%"><?php echo esc_html( $wp_query_options ); ?></textarea>
            </p>

            <p>
                <label for="<?php echo esc_attr( $this->get_field_id('widget_output_template') ); ?>"><?php _e('Widget Output Template', 'functionality-for-blogpostlite-theme'); ?></label> <a title="View Documentation" target="_blank" href="http://www.pjgalbraith.com/2011/08/recent-posts-plus/">(?)</a><br />
                <textarea id="<?php echo esc_attr( $this->get_field_id('widget_output_template') ); ?>" name="<?php echo esc_attr( $this->get_field_name('widget_output_template') ); ?>" style="width:222px; height: 100px; font-size: 80%"><?php echo esc_html( $widget_output_template ); ?></textarea>
            </p>
