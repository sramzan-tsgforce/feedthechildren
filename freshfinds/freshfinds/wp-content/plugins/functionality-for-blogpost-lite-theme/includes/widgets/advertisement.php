...
class vh_advertisement extends WP_Widget {
	...
	public function vh_advertisement() {
		$widget_opts = array(
			...
			'description' => __('Displays advertisement', 'functionality-for-blogpostlite-theme')
		);
		parent::__construct('vh_advertisement', __('BlogPost - Advertisement', 'functionality-for-blogpostlite-theme'), $widget_opts);
	}
	...
	public function widget($args, $instance) {
		...
		for ($i = 1; $i <= $count; $i++) {
			...
			<div class="advertisement-container">
				<a href="<?php echo esc_url( $link ); ?>" rel="nofollow" target="_top" class="no_thickbox" title="<?php _e('Advertisement', 'functionality-for-blogpostlite-theme') ?>">
					<img src="<?php echo esc_url( $image ); ?>" alt="<?php _e('Advertisement', 'functionality-for-blogpostlite-theme') ?>" />
				</a>
			</div>
			<?php
		}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('count') ); ?>"><?php _e('How many ads to display?', 'functionality-for-blogpostlite-theme'); ?></label>
		...
		<div class="advertisement_container">
			...
			for ($i = 1; $i <= $this->max_ads; $i++) {
				...
				<div class="hidden_container" <?php if ($i > $count) { ?>style="display:none"<?php } ?>>
					...
					<p>
						<label for="<?php echo esc_attr( $this->get_field_id($ad_image) ); ?>"><?php _e('Image URL:', 'functionality-for-blogpostlite-theme'); ?></label>
					...
					<p>
						<label for="<?php echo esc_attr( $this->get_field_id($ad_link) ); ?>"><?php _e('Link:', 'functionality-for-blogpostlite-theme'); ?></label>
					...
					<p>
						<em><?php _e("Example: <code>http://www.example.com</code>", 'functionality-for-blogpostlite-theme'); ?></em>
					</p>
				</div>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "vh_advertisement" );' ) );